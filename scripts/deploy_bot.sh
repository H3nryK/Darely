...
if [ "$NETWORK" = "playground" ]; then
    ...
    CANISTER_ID=$(dfx canister id Darely_Bot --network playground 2>/dev/null)
    
    if [ $? -eq 0 ] && [ ! -z "$CANISTER_ID" ]; then
        echo "Canister exists. Attempting upgrade..."
        # Try to upgrade first
        if ! dfx deploy Darely_Bot --playground --argument "$ARGS" --mode upgrade; then
            echo "Upgrade failed. The canister might have expired. Creating a new one..."
            # If upgrade fails, create a new canister
            dfx deploy Darely_Bot --playground --argument "$ARGS" --mode install
        fi
    else
    ...
    CANISTER_ID=$(dfx canister id Darely_Bot --network playground)
    ...
    ./utils/deploy.sh Darely_Bot OnchainBot $MODE "$ARGS" $NETWORK
fi